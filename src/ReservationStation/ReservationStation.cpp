...
bool ReservationStation::TryTransmit(const instruct &new_ins, const int &entry_num, const RegisterFile &RF) {
    if (IsLS(new_ins.ins_type)) { //指令为load或store，需要访存，必须顺序执行
        ...
        a.imm = new_ins.imm;
        if (RF.regs[new_ins.rs1].depend == -1) { //处理第一个寄存器
            a.v1 = RF.regs[new_ins.rs1].value;
        } else {
            a.q1 = RF.regs[new_ins.rs1].depend;
        }
        if (new_ins.ins_type[0] == 's') { //Store
            if (RF.regs[new_ins.rs2].depend == -1) { //处理第二个寄存器
                a.v2 = RF.regs[new_ins.rs2].value;
            } else {
                a.q2 = RF.regs[new_ins.rs2].depend;
            }
        }
        RS2.EnQueue(a); //发射
        return true;
...
void ReservationStation::TimeTraversal(std::vector<int> &a1) {
    for (int i = 0; i < 5; i++) {
        if (RS1[i].time > 0) {
            ...
            a1.push_back(i);
            RS1[i].time = -2; //把这个单元置空
        }
    }
}

//对时间已为0的指令执行ready操作,nums中是已经ready的指令的编号
//将结果记入RoB中的result;
//RoB中的status=ready
void ReservationStation::SetReady(const std::vector<int> &nums1, ReorderBuffer &RoB) {
    ...
    for (auto i: nums1) {
        ...
        if (RS1[i].type == "lui") {
            ...
            RoB.queue.queue[entry_num].result1 = RS1[i].v1 < (unsigned int) RS1[i].imm; //rd
            ...
            RoB.queue.queue[entry_num].result1 = RS1[i].v1 < unsigned(RS1[i].v2); //rd
...
void ReservationStation::CDB(const ReorderBuffer &RoB) { //如果寄存器所依赖的RoB中指令已经Ready，则将结果置于v1/v2中，令q1/q2=-1
    for (auto &i: RS1) {
        if (i.q1 != -1 && RoB.queue.queue[i.q1].state == ready) {
            i.v1 = RoB.queue.queue[i.q1].result1; //result1储存的是寄存器值
            i.q1 = -1;
        }
        if (i.q2 != -1 && RoB.queue.queue[i.q2].state == ready) {
    ...
    for (int i = RS2.front, j; i != RS2.rear; i = j) {
        ...
        if (RS2.queue[j].q1 != -1 && RoB.queue.queue[RS2.queue[j].q1].state == ready) {
            RS2.queue[j].v1 = RoB.queue.queue[j].result1;
            RS2.queue[j].q1 = -1;
        }
        if (RS2.queue[j].q2 != -1 && RoB.queue.queue[RS2.queue[j].q2].state == ready) {
            RS2.queue[j].v2 = RoB.queue.queue[j].result1;
            RS2.queue[j].q2 = -1;
        }
    }
}

void ReservationStation::SetExecute() { //把寄存器的值全部确定的指令放入store_buffer
    for (auto &i: RS1) {
        if (i.q1 == -1 && i.q2 == -1) {
            i.time = 1;
        }
    ...
    for (int i = RS2.front, j; i != RS2.rear; i = j) {
        j = (i + 1) % RS2.MaxSize;
        if (RS2.queue[j].q1 == -1 && RS2.queue[j].q2 == -1) {
            SLEle a;
            a.address = RS2.queue[j].imm + RS2.queue[j].v1;
            a.entry_num = RS2.queue[j].entry;
            a.type = RS2.queue[j].type;
            if (RS2.queue[j].type[0] == 's') { //store指令，值已经可以确定
                if (RS2.queue[j].type == "sb") {
                    a.value = RS2.queue[j].v2 & 127; //取0-7bits
                } else if (RS2.queue[j].type == "sh") {
                    a.value = RS2.queue[j].v2 & 65535; //取0-15bits
                } else {
                    a.value = RS2.queue[j].v2; //取0-32bits
                }
            }
...
void ReservationStation::ExecuteLSBuffer(ReorderBuffer &RoB, const Memory &memory) {
    if (store_buffer.Empty()) return;
    if (store_buffer.Front().time > 0) {
        --store_buffer.Front().time;
    } else if (store_buffer.Front().time == 0) { //已经执行完毕
        ...
        if (store_buffer.Front().type[0] == 's') RoB.store_commit = false; //本条store指令还未commit
    } else if (RoB.store_commit) { //只有store指令已经commit了才可以进行后面的load&store操作
